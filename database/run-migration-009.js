...
async function runMigration() {
  ...
  try {
    // Read migration file
    const migrationPath = path.join(__dirname, 'migrations', '009_ab_test_metrics.sql');
    const migrationSQL = fs.readFileSync(migrationPath, 'utf8');

    console.log('Running migration 009_ab_test_metrics.sql...');
    ...
    const tablesCheck = await pool.query(`
      ...
      AND table_name = 'ab_test_metrics'
    `);
    ...
    const indexesCheck = await pool.query(`
      SELECT indexname
      FROM pg_indexes
      WHERE tablename = 'ab_test_metrics'
    ...
    console.log('\n✅ Migration 009 completed and verified successfully!');
